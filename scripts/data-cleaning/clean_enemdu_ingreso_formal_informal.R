# ============================================================
# clean_enemdu_ingreso_formal_informal.R
# Resume ingresos individuales de trabajadores formales e
# informales en la ENEMDU marzo 2026, con ponderacion fexp.
# Requiere: data/raw/enemdu/enemdu_persona_2026_03.sav
# Guarda:   outputs/tables/enemdu_ingreso_formal_informal_2026_03_summary.xlsx
#           outputs/tables/enemdu_ingreso_formal_informal_2026_03_summary.html
# ============================================================
# Ejecutar desde la raiz del proyecto:
#   Rscript scripts/data-cleaning/clean_enemdu_ingreso_formal_informal.R
# ============================================================

source("scripts/packages.R")
ensure_packages(c("dplyr", "tidyr", "haven", "readr", "openxlsx", "gt", "Hmisc"))

input_path <- "data/raw/enemdu/enemdu_persona_2026_03.sav"
xlsx_path <- "outputs/tables/enemdu_ingreso_formal_informal_2026_03_summary.xlsx"
html_path <- "outputs/tables/enemdu_ingreso_formal_informal_2026_03_summary.html"

enemdu_persona <- read_sav(input_path) %>%
  mutate(
    across(
      c(p63, p66, p68b, p69, p71b, p72b, p73b, p74b, p76, p78),
      ~ as.numeric(zap_labels(.x))
    ),
    across(
      c(p63, p66, p68b, p69, p71b, p72b, p73b, p74b, p76, p78),
      ~ na_if(.x, 999999)
    ),
    across(
      c(p63, p66, p68b, p69, p71b, p72b, p73b, p74b, p76, p78),
      ~ na_if(.x, -1)
    ),
    across(
      c(p63, p66, p68b, p69, p71b, p72b, p73b, p74b, p76, p78),
      ~ replace_na(.x, 0)
    ),
    empleo = as.numeric(empleo),
    p42 = as.numeric(zap_labels(p42)),
    p432 = as.numeric(zap_labels(p432)),
    p44f = as.numeric(zap_labels(p44f)),
    p48 = as.numeric(zap_labels(p48)),
    p49 = as.numeric(zap_labels(p49)),
    p61b1 = as.numeric(zap_labels(p61b1)),
    formal = case_when(
      empleo != 1 ~ NA_real_,
      p42 %in% c(1, 2, 3, 4, 10) & (p44f == 1 | p432 == 1) ~ 1,
      p42 %in% c(1, 2, 3, 4, 10) & (p44f == 2 | p432 %in% c(2, 3)) ~ 0,
      p42 %in% c(5, 6) & (p49 == 1 | p48 %in% c(1, 2) | p61b1 %in% c(1, 2, 3, 4)) ~ 1,
      p42 %in% c(5, 6) & (p49 == 2 | p48 == 3 | p61b1 == 5) ~ 0,
      p42 %in% c(7, 8, 9) ~ 0,
      TRUE ~ NA_real_
    ),
    ingreso_laboral_primaria = p63 + p66 + p68b,
    ingreso_laboral_secundaria = p69,
    ingreso_laboral_total = ingreso_laboral_primaria + ingreso_laboral_secundaria,
    ingreso_capital_inversiones = p71b,
    transferencias_prestaciones = p72b + p73b + p74b + p76 + p78,
    ingreso_total_persona =
      ingreso_laboral_total +
      ingreso_capital_inversiones +
      transferencias_prestaciones
  )

n_hogares <- enemdu_persona %>%
  summarise(n_hogares = n_distinct(id_hogar)) %>%
  pull(n_hogares)

enemdu_trabajadores <- enemdu_persona %>%
  filter(empleo == 1, !is.na(formal)) %>%
  mutate(formalidad = if_else(formal == 1, "Formal", "Informal"))

summary_total <- enemdu_trabajadores %>%
  summarise(
    `Número de hogares` = n_hogares,
    `Promedio` = weighted.mean(ingreso_total_persona, w = fexp, na.rm = TRUE),
    `Cuartil 1` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.25, na.rm = TRUE)[[1]],
    `Mediana` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.50, na.rm = TRUE)[[1]],
    `Cuartil 3` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.75, na.rm = TRUE)[[1]],
    `Quintil 1 (p20)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.20, na.rm = TRUE)[[1]],
    `Quintil 2 (p40)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.40, na.rm = TRUE)[[1]],
    `Quintil 3 (p60)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.60, na.rm = TRUE)[[1]],
    `Quintil 4 (p80)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.80, na.rm = TRUE)[[1]]
  ) %>%
  pivot_longer(
    cols = everything(),
    names_to = "metrica",
    values_to = "valor"
  ) %>%
  mutate(grupo = "Total")

summary_formalidad <- enemdu_trabajadores %>%
  group_by(formalidad) %>%
  summarise(
    `Número de hogares` = n_hogares,
    `Promedio` = weighted.mean(ingreso_total_persona, w = fexp, na.rm = TRUE),
    `Cuartil 1` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.25, na.rm = TRUE)[[1]],
    `Mediana` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.50, na.rm = TRUE)[[1]],
    `Cuartil 3` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.75, na.rm = TRUE)[[1]],
    `Quintil 1 (p20)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.20, na.rm = TRUE)[[1]],
    `Quintil 2 (p40)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.40, na.rm = TRUE)[[1]],
    `Quintil 3 (p60)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.60, na.rm = TRUE)[[1]],
    `Quintil 4 (p80)` = Hmisc::wtd.quantile(ingreso_total_persona, weights = fexp, probs = 0.80, na.rm = TRUE)[[1]],
    .groups = "drop"
  ) %>%
  pivot_longer(
    cols = -formalidad,
    names_to = "metrica",
    values_to = "valor"
  ) %>%
  rename(grupo = formalidad)

summary_table <- bind_rows(summary_total, summary_formalidad) %>%
  pivot_wider(
    names_from = grupo,
    values_from = valor
  )

dir.create("outputs/tables", showWarnings = FALSE, recursive = TRUE)

wb <- createWorkbook()
addWorksheet(wb, "summary")
writeData(wb, "summary", summary_table)

header_style <- createStyle(
  textDecoration = "bold",
  halign = "center",
  fgFill = "#D9E2F3",
  border = "Bottom"
)

currency_style <- createStyle(numFmt = "$#,##0.00")
count_style <- createStyle(numFmt = "#,##0")

addStyle(
  wb,
  "summary",
  style = header_style,
  rows = 1,
  cols = 1:ncol(summary_table),
  gridExpand = TRUE
)

addStyle(
  wb,
  "summary",
  style = currency_style,
  rows = 3:(nrow(summary_table) + 1),
  cols = 2:ncol(summary_table),
  gridExpand = TRUE
)

addStyle(
  wb,
  "summary",
  style = count_style,
  rows = 2,
  cols = 2:ncol(summary_table),
  gridExpand = TRUE
)

setColWidths(wb, "summary", cols = 1, widths = 22)
setColWidths(wb, "summary", cols = 2:ncol(summary_table), widths = 14)
freezePane(wb, "summary", firstRow = TRUE)

saveWorkbook(wb, xlsx_path, overwrite = TRUE)

summary_table %>%
  gt() %>%
  fmt_number(columns = c(Total, Formal, Informal), rows = metrica == "Número de hogares", decimals = 0) %>%
  fmt_currency(columns = c(Total, Formal, Informal), rows = metrica != "Número de hogares", currency = "USD", decimals = 2) %>%
  cols_label(
    metrica = "Métrica",
    Total = "Total",
    Formal = "Formal",
    Informal = "Informal"
  ) %>%
  tab_header(
    title = "ENEMDU marzo 2026",
    subtitle = "Resumen ponderado del ingreso individual de trabajadores formales e informales"
  ) %>%
  tab_source_note(
    source_note = "Fuente: ENEMDU, microdatos marzo 2026. Cálculos ponderados. Por: Daniel Sánchez."
  ) %>%
  gtsave(html_path)

print(summary_table)
message("Hogares distintos en la base: ", n_hogares)
message("Guardado: ", xlsx_path)
message("Guardado: ", html_path)

